...
async function main() {
    ...
    const foldContract = await AaveFold.deploy({ gasPrice: ethers.utils.parseUnits('90', 'gwei') });
    ...
    const folder = await foldContract.unFoldPosition(DAIToken.address, lendingAddress);
